...
### 3. 운영 영향을 차단하는 권한 설계 (실습이 아닌 배포 전략)
- **문제**: dbt를 처음 도입하는 단계에서는 시행착오가 필연적인데, 이 과정이 운영 스키마(`PUBLIC`)에 그대로 반영되면 실제 대시보드·서비스 로직에 영향을 줄 수 있는 리스크가 있었습니다.
- **해결**: `DBT_SANDBOX`라는 별도 스키마를 신설하고, 여기에만 `CREATE VIEW`/`CREATE TABLE` 권한을 최소 부여했습니다. 이후 dbt profile을 `dev`(sandbox)와 `prod`(운영용 전용 마트 스키마) 두 타겟으로 분리하고, 기본값은 항상 `dev`를 가리키도록 설계해 `--target prod`를 명시적으로 지정하지 않는 한 운영에 반영될 수 없는 이중 안전장치를 만들었습니다.
- **의도**: 이건 "학습을 위한 격리"가 아니라, 실제로 검증되지 않은 로직이 배포 파이프라인 없이 프로덕션에 스며드는 걸 막기 위한 최소 권한(least privilege) 설계였습니다. 이후 검증이 끝난 모델만 `--target prod`로 승격시키는 흐름 자체가, 신규 도구를 프로덕션에 안전하게 들여오는 하나의 배포 전략으로 이어졌습니다 (상세는 `04_DBT_LIVE_VALIDATION` 참고).
