...
이러한 직관적 액션을 시스템화하여 **"쿠폰 고관여 체리피커 유저", "주기적으로 이용하나 앱 접속 후 세탁을 맡기지 않는 이탈 위역 유저"** 등으로 구체화하기 위해 13개 행동 피처를 기반으로 모델 벤치마킹을 수행했습니다.
...
* **최종 선택 (GMM, Gaussian Mixture Model):** 각 군집이 고유한 평균과 분산을 바탕으로 유연한 **'타원형(Elliptical)'** 공분산 구조를 가질 수 있어, 넓게 퍼진 VVIP 아웃라이어 데이터까지 노이즈 처리 없이 완벽하게 포용할 수 있는 유일한 대안이었습니다.
...
2. **Feature Engineering & Normalization (전처리 및 정규화)**
   ...
   - GMM 모델의 '각 군집은 정규분포를 따른다'는 수학적 가정을 만족시키기 위해, 비대칭 피처에 **`np.log1p` 및 `np.sqrt` 중첩 변환**을 선제적으로 수행하여 정규성(Normality) 확보.
...
4. **Deploy & Automation (배포 및 자동화)**
   - 전체 파이프라인 코드를 Docker 컨테이너 이미지로 빌드하여 AWS ECR에 푸시 후, **AWS Lambda + EventBridge**를 연동해 매월 자동으로 무중단 배치 실행.
5. **Integration (서버 및 CRM 연동)**
   - 자동 분류 및 매핑된 유저별 군집 결과는 CSV 형태의 데이터 스냅샷으로 **AWS S3 버킷**에 안전하게 적재.
   - 사내 백엔드 시스템 및 다운스트림 CRM 마케팅 툴이 해당 S3 오브젝트를 직접 Fetch하여 타겟 마케팅에 즉각 활용할 수 있는 연동 아키텍처 구축.
...
- **Resolution (아키텍처 분리 및 스냅샷 관리):** 실시간 처리 데이터베이스와 분석용 마케팅 적재 환경을 철저히 분리했습니다. Lambda 최종 단계에서 산출된 결과를 유저 테이블에 직접 하드코딩 적재하는 대신, 해당 시점의 고유 날짜 타임스탬프를 포함한 CSV 스냅샷 형태로 S3 버킷에 격리 저장하는 방식을 채택했습니다. 사내 다운스트림 서버에서 필요할 때마다 이 S3 파일 스냅샷을 뷰(View)의 개념으로 활용하게 함으로써 인프라의 안정성을 원천적으로 확보했습니다.
