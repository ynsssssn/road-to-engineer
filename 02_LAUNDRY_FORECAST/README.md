...
- **XGBoost/LSTM 배제:** 1,000건 수준의 스몰 데이터 환경에서 과적합 위험이 높았습니다.
- **ARIMA 배제:** 명절 등 물량이 급변하는 공휴일 효과 및 추세 변화점 대응에 취약했습니다.
- **Prophet 채택:** 계절성 반영에 강하며, 단기 기상 변화를 외부 회귀 변수로 손쉽게 결합할 수 있어 최종 선택했습니다.
...
권역별 미래 기온을 예측한 뒤 전주 대비 기온 변화(1도/2도/3도) 수준을 계산하고, 비즈니스 영향도가 발생하는 다중 임계값 조건을 추출하여 메인 수요 예측 모델의 외부 회귀 변수로 주입합니다.
...
무거운 시계열 모델(Prophet)을 AWS Lambda 환경에서 안정적으로 운영하고 결과를 대시보드에 적재하기 위해 다음 문제들을 단계별로 해결했습니다.

1. **시도 A (Docker 이미지 업로드 중 Timeout 이슈):** 
   - **문제:** `prophet`, `snowflake` 등 무거운 라이브러리를 Global에서 import하다 부팅 제한시간(10초)을 초과하는 현상 발생.
   - **해결:** `lambda_handler` 및 각 개별 함수 내부에서 호출하여 초기 부팅 타임아웃을 회피함.
...
3. **시도 C (과거 예측 히스토리 유실 방지):** 
   - **문제:** 매일 미래 60일 데이터가 덮어씌워져 과거 시점의 예측 수치가 날아가는 현상 발생.
   - **해결:** 대시보드 시트와 데이터 적재 시트를 분리하고, 주 1회 Google Apps Script를 통해 확정된 과거 예측값을 '텍스트 형태(값)로 고정'하는 로직을 적용해 데이터 히스토리 보존.
